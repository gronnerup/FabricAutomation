...
parser.add_argument("--branch_name", required=False, default=os.environ.get('GITHUB_REF_NAME'), help="The name of the Git feature branch to operate on. Used for workspace setup, automation, and CI/CD logic. Defaults to a predefined variable `branch_name`.")
            ...
            if fabcli.run_command(f"exists {workspace_name_escaped}.Workspace").replace("*", "").strip().lower() == "false":
                ...
                if git_settings:
                    if git_settings.get("gitProviderDetails").get("gitProviderType").lower() == "github":
                        misc.print_info(f"  - Setting up Git integration (GitHub) for workspace... ", end="")
                        if fabcli.connection_exists(git_settings.get("myGitCredentials").get("connectionId")):
                            ...
                            misc.print_error(f"Connection id {git_settings.get("myGitCredentials").get("connectionId")} not found. Skipping Git integration setup.")
                    else:
                        misc.print_warning(f"  - Skipping setup of Git integration as SPN is not supported for Azure DevOps git integration! ", end="")       
            else:
